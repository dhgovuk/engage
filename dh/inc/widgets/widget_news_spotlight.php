...
class DH_Widget_News_Spotlight extends DH_Super_Widget {
  /**
   * Constructor.
   */
  public function __construct() {
    parent::__construct( 'widget_dh_news_spotlight', __( 'DH: News Spotlight', 'dh' ), array(
        'description' => __( 'Use this widget to display News Spotlights (Differs from regular spotlights in that they show the date, and they only show news!).', 'dh' ),
    ) );
  }
  ...
  public function widget( $args, $instance ) {
    ...
    global $eligible_news;

    if ( ! $eligible_news ) {
      $eligible_news = get_posts( array( 'post_type' => array('news'), 'numberposts' => -1, 'orderby' => 'ID' ) );
    }

    if ( $instance['spotlight_post'] && $spotlight_post = get_post( $instance['spotlight_post'] ) ) {
      // Cool, we got out spotlight spot

      // We remove it from the set of $eligible_news
      foreach ( $eligible_news as $index => $eligible_news_item ) {
        if ( $eligible_news_item->ID == $instance['spotlight_post'] ) {
          unset( $eligible_news[$index] );
        }
      }
    }
    else {
      // Otherwise, get the newest post
      $spotlight_post = $eligible_news[0];

      // Remove it from the set of $eligible_news
      unset( $eligible_news[0] );
    }
    // Re-index the set of $eligible_news
    $eligible_news = array_values( $eligible_news );
    ...
    <div class="grid-<?php echo $width . '-' . $row_width; ?>">
      <div class="block-spotlight block-spotlight-<?php echo $instance['spotlight_post']; ?>">
        ...
        <div class="comments-meta">
          <div class="comments-date"><?php echo date('j F Y  |  H:iA', strtotime($spotlight_post->post_date)); ?></div>
        </div>
        <p><?php echo $text; ?></p>
      </div>
    </div>
  ...
  function form( $instance, $fields = Array() ) {
    ...
    $all_posts = get_posts( array( 'post_type' => array( 'news' ), 'numberposts' => -1, 'orderby' => 'ID' ) );
    ?>
      <p>
